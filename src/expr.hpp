...
struct Quote : ExprBase {
  Syntax s;//store the grammar structure
  Quote(const Syntax &);//Constructor
  virtual Value eval(Assoc &) override;
};
...
struct If : ExprBase {
  Expr cond;//(condition) predicate
  Expr conseq;//(consequent) then_value
  Expr alter;//(alternative) else_value
